#pragma once
/*Форма выбора действий в режиме учащегося
* Единственно возможный экземпляр создается в конструкторе CurrentApplicationContextъ
* Закрытие формы через "крестик" приводит к закрытию приложения
* Для продолжения работы приложения без этой формы использовать метод Hide()
*/
