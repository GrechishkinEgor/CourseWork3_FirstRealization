#pragma once
/*Форма отвечает за выбор между двумя режимами работы: преподавателя или студента.
* Закрытие формы через метод Close() в "крестике" или соответствующей кнопке интерфейса завершает приложение целиком.
* Для продолжения работы приложения необходимо скрыть форму через метод Hide()
* Форма конструируется один раз - в конструкторе CurrentApplicationContext.
* Указатель на единственно возможный экземпляр формы хранится в CurrentApplicationContext
*/
