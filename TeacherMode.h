#pragma once
/*Стартовое окно приложения, предназначенное для выбора режима работы.
* Единственно возможных экземпляр создается в конструкторе CurrentApplicationContext.
* Закрытие окна закрывает все приложение.
* Для предотвращения закрытия всего приложения использовать метод Hide().
*/
